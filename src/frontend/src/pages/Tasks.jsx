            ...
            <div key={task.id}>
              <div className="task-item">
                ...
                <div className="task-actions">
                  ...
                  <button className="btn btn-secondary btn-sm" onClick={() => handleDecompose(task.id)} title="AI Decompose">🤖</button>
                ...
                <div className="subtask-list">
                  {expandedTask.subtasks.map((st) => (
                    <div key={st.id} className="subtask-item">
                      <span className={`badge badge-${st.status}`} style={{ fontSize: 10 }}>{st.status}</span>
                      <span>{st.title}</span>
                      {st.estimated_minutes && <span style={{ color: "var(--text-muted)", fontSize: 12 }}>({st.estimated_minutes}min)</span>}
                      {st.status === "pending" && (
                        <button className="btn btn-success btn-sm" onClick={() => handleStatusChange(st.id, "completed")} style={{ marginLeft: "auto" }}>✓</button>
                      )}
                    </div>
                  ))}
                </div>
